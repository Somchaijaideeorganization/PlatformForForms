...
export const fetchRunningMigrationWithSuccessfulRollback = (url, request) => {
    if (url === `${baseUrl}/migrations/status` && request.method === 'GET') {
        return {
            ok: true,
            json: async () => {
...
describe('migrate rollback', () => {
    ...
    test('correctly starts the migration rollback job, if an active migration is found', async () => {
        ...
        await command.run();
        expect(log).toHaveBeenCalledWith(`Migration rollback started with Job ID mig_job_cqfltm0knc8jbigtjb9g. Please use the "xata migration status main" command to check its status`);
    });
});
